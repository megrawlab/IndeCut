...
*                                                                            *
*   Copyright (1984-2002) Brendan McKay.  All rights reserved.  Permission   *
...
dispatchvec dispatch_graph;   /* initialised in naugraph.c, but that file
                                 need not be included if this dispatch
                                 vector is not used */
...
static void (*usernodeproc)(graph*,int*,int*,int,int,int,int,int,int),
            (*userautomproc)(int,permutation*,int*,int,int,int),
            (*userlevelproc)
              (int*,int*,int,int*,statsblk*,int,int,int,int,int,int),
	    (*tcellproc)(graph*,int*,int*,int,int,set*,
      ...
      set *ws_arg, int worksize, int m_arg, int n_arg, graph *canong_arg)
{
	...
	if (options->dispatch) dispatch = *(options->dispatch);
        else                   dispatch = dispatch_graph;
...
#endif
	{
            ...
            stats->invarsuclevel =
                 (invarsuclevel == INFTY ? 0 : invarsuclevel);
