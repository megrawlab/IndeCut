...
int main(int argc, char** argv){
	...
	if (num_r_nets<0){
	    errors_array[1] = new string("Number of random networks should be a nonnegative integer\n");
	}
    ...
    // (mm) open file rgfile for output of random graphs
    std::ofstream rgfile((outputfile+".randgraphs").c_str(), ios::out);
    rgfile.close();
	...
	for (int nets_ctr = 0; nets_ctr < total_num_nets; ++nets_ctr){
		...
		/*
		// (mm) In this case, only want to output the random graphs, don't need to enumerate subgraphs
		...
		*/
		...
		for (hash_map < graphcode64, uint64 >::const_iterator iter = inter_result.begin();
			...
			if (num_r_nets != 0) {
				...
				// write the resulting random graphs to a file (mm)
				/* ------------- */
				if (!(is_orig)) {
				  rgfile.open((outputfile+".randgraphs").c_str(),ios::app);
				  for (hash_map < edge, edgetype >::const_iterator iter = maing.edges.begin();
				       iter != maing.edges.end(); ++iter) {
