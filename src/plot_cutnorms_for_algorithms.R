...
if (length(args) < 1) {
    cat("Usage ./merge_two_sets.R [cutnorm_infile.txt]\n")
    quit(save="no", status=1)
}

cutnorm_infile <- "Downloads/human_net_cutnorms.txt"
...
g1 <- ggplot(data, aes(y = Algorithm)) + labs(x = "cutnorm", y = "algorithm")  + 
  ...
  geom_point(aes(x = data$lower_bound, color = Algorithm), size = 4, shape = 'I') +
  geom_point(aes(x = data$upper_bound, color = Algorithm), size = 4, shape = 'I') + 
