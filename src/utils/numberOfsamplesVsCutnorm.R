#!/usr/bin/Rscript
graphname <- "dronet_TF-TF"
nsamples_total <- 5000
nsubsamples <- 50
zcutnorm <- 461.749790
displayName <- "Drosophila TF->TF"

path <- "samples_vs_cutnorms_out/"
...
g1file <- paste(graphname, "_alltimes_allAlgs.png", sep = "")
ggsave(file=g1file, threeallg)

mdata_20 <- subset(mdata, (mdata$sampleCount > 50 & mdata$sampleCount < 1200))
